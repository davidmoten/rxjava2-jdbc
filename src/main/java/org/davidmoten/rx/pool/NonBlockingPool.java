...
public final class NonBlockingPool<T> implements Pool<T> {
	...
	public NonBlockingPool(Callable<T> factory, Predicate<T> healthy, Consumer<T> disposer, int maxSize,
			long retryDelayMs, MemberFactory<T, NonBlockingPool<T>> memberFactory, Scheduler scheduler) {
	...
	public static class Builder<T> {
		...
		public NonBlockingPool<T> build() {
			return new NonBlockingPool<T>(factory, healthy, disposer, maxSize, retryDelayMs, memberFactory, scheduler);
		}
	}
