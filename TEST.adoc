...
The test database has a few tables (see script[link:src/main/resources/database-test.sql]) including `Person` and `Address` with three rows in `Person` and two rows in `Address`:
